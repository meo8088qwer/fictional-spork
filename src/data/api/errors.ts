export type PlanLimitCode =
  | 'FREE_PLAN_EVENT_LIMIT_REACHED'
  | 'STUDENT_LIMIT_REACHED'
  | 'BASIC_FEATURE_LOCKED'
  | 'PRO_FEATURE_LOCKED';
...
export function planLimitMessage(code: PlanLimitCode): string {
  if (code === 'FREE_PLAN_EVENT_LIMIT_REACHED') {
    return '현재 플랜의 종목 추가 한도에 도달했습니다. 더 추가하려면 플랜을 업그레이드해 주세요.';
  }
  if (code === 'BASIC_FEATURE_LOCKED') {
    return '이 기능은 베이직 플랜부터 이용할 수 있어요. 업그레이드하면 바로 사용할 수 있습니다.';
  }
  if (code === 'PRO_FEATURE_LOCKED') {
    return '이 기능은 PRO 플랜부터 이용할 수 있어요. 업그레이드하면 바로 사용할 수 있습니다.';
  }
